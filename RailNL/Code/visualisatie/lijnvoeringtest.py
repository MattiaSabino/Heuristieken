...
def latenwevisualiseren(inputtraject):
	# Lijsten
	stations 		= []
	xcoordinates 	= []
	ycoordinates 	= []
	...
	alleTrajecten = inputtraject

	# Dit is een visualisatie aanvraag voor 0 trajecten, dit levert een lege lijnvoering op:
	#alleTrajecten = [[]]

	def visualisatie(x):
		with open('C:\Users\Gebruiker\Desktop\Programmeer minor\Heuristieken\StationsNationaal.csv', 'rb') as f:
			reader = csv.reader(f)
			for row in reader:
		...
		with open('C:\Users\Gebruiker\Desktop\Programmeer minor\Heuristieken\ConnectiesNationaal.csv', 'rb') as f:
			reader = csv.reader(f)
			for row in reader:
